...
struct s2m_rtc_info {
	...
	u8			audr_mask;
#ifdef CONFIG_SEC_PM
	struct delayed_work	rtc_check_work;
#endif
};
...
static int rtc_check;
static u8 rtc_check_data[NR_RTC_CNT_REGS];
static bool rtc_check_first = true;
#endif
...
static int s2m_rtc_read_time(struct device *dev, struct rtc_time *tm)
{
	...
	ret = s2mpu08_bulk_read(info->i2c, S2MP_RTC_REG_SEC, NR_RTC_CNT_REGS,
			data);
	if (ret < 0) {
...
static int s2m_rtc_read_hrtime(struct device *dev, struct rtc_hrtime *tm)
{
	...
	ret = s2mpu08_bulk_read(info->i2c, S2MP_RTC_REG_MSEC,
...
static int s2m_rtc_set_time(struct device *dev, struct rtc_time *tm)
{
	...
	mutex_lock(&info->lock);
	ret = s2mpu08_bulk_write(info->i2c, S2MP_RTC_REG_SEC, NR_RTC_CNT_REGS,
			data);
	if (ret < 0) {
...
static int s2m_rtc_check_rtc_time(struct s2m_rtc_info *info)
{
	...
	if (abs(rtc_time - sys_time.tv_sec) > 2) {
		...
		ret = s2mpu08_bulk_write(info->i2c, S2MP_RTC_REG_SEC,
...
static int s2m_rtc_read_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	ret = s2mpu08_bulk_read(info->i2c, S2MP_RTC_REG_A0SEC, NR_RTC_CNT_REGS,
			data);
	if (ret < 0) {
	...
	ret = s2mpu08_read_reg(info->pmic_i2c, reg, &val); /* i2c for PM */
	if (ret < 0) {
...
static int s2m_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	if (info->alarm_check) {
		...
		ret = s2mpu08_bulk_write(info->i2c, S2MP_RTC_REG_A0SEC, NR_RTC_CNT_REGS,
				data);
		if (ret < 0) {
						...
						struct s2mpu08_platform_data *pdata)
{
	...
	if (pdata->osc_bias_up >= 0) {
		ret = s2mpu08_update_reg(info->i2c, S2MP_RTC_REG_CAPSEL,
	...
	if (pdata->cap_sel >= 0) {
		s2mpu08_update_reg(info->i2c, S2MP_RTC_REG_CAPSEL,
	...
	if (pdata->osc_xin >= 0) {
		s2mpu08_update_reg(info->i2c, S2MP_RTC_REG_OSCCTRL,
...
static bool s2m_is_jigonb_low(struct s2m_rtc_info *info)
{
	...
	ret = s2mpu08_read_reg(info->i2c, reg, &val);
	if (ret < 0) {
						...
						struct s2mpu08_platform_data *pdata)
{
	...
	ret = s2mpu08_write_reg(info->i2c, S2MP_RTC_REG_WTSR_SMPL, val);
	if (ret < 0) {
					...
					struct s2mpu08_platform_data *pdata)
{
	...
	ret = s2mpu08_update_reg(info->i2c, S2MP_RTC_REG_WTSR_SMPL, 0,
				...
				struct s2mpu08_platform_data *pdata)
{
	...
	ret = s2mpu08_read_reg(info->i2c, S2MP_RTC_REG_CTRL, &ctrl_val);
	if (ret < 0) {
	...
	ret = s2mpu08_read_reg(info->i2c, S2MP_RTC_REG_CAPSEL, &capsel_val);
	if (ret < 0) {
	...
	data = MODEL24_MASK;
	ret = s2mpu08_write_reg(info->i2c, S2MP_RTC_REG_CTRL, data);
	if (ret < 0) {
	...
	capsel_val |= 0xf0;
	ret = s2mpu08_write_reg(info->i2c, S2MP_RTC_REG_CAPSEL, capsel_val);
	if (ret < 0) {
...
static void rtc_check_thread(struct work_struct *work)
{
	struct s2m_rtc_info *info = container_of(work, struct s2m_rtc_info,
			rtc_check_work.work);
	int i;
	u8 data[NR_RTC_CNT_REGS];
	int ret;
	...
	ret = s2mpu08_bulk_read(info->i2c, S2MP_RTC_REG_SEC,
			NR_RTC_CNT_REGS, data);
	...
	if (rtc_check_first) {
		for (i = 0; i < NR_RTC_CNT_REGS; i++)
			rtc_check_data[i] = data[i];

		rtc_check_first = false;

		schedule_delayed_work(&info->rtc_check_work,
				msecs_to_jiffies(15000));
	} else {
		for (i = 0; i < NR_RTC_CNT_REGS; i++) {
			if (rtc_check_data[i] != data[i]) {
				rtc_check = 1;
				goto out;
			}
		}
...
static ssize_t show_rtc_check(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", rtc_check);
}

static DEVICE_ATTR(rtc_check, 0444, show_rtc_check, NULL);
#endif

static int s2m_rtc_probe(struct platform_device *pdev)
{
	struct s2mpu08_dev *iodev = dev_get_drvdata(pdev->dev.parent);
	struct s2mpu08_platform_data *pdata = dev_get_platdata(iodev->dev);
	...
	INIT_DELAYED_WORK(&info->rtc_check_work, rtc_check_thread);
	schedule_delayed_work(&info->rtc_check_work, 0);

	/* create sysfs group */
	ret = sysfs_create_file(power_kobj, &dev_attr_rtc_check.attr);
	if (ret)
		pr_err("%s: failed to create rtc_status attribute file\n",
				__func__);
#endif
