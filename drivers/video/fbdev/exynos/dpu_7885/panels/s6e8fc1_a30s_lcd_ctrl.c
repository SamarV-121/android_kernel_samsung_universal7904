...
{
	int ret = 0, opr_status = OPR_STATUS_15P, acl_status = ACL_STATUS_15P;
#if defined(CONFIG_SUPPORT_MASK_LAYER)
...
update:
	DSI_WRITE(SEQ_GPARA_ACL, ARRAY_SIZE(SEQ_GPARA_ACL));
...
#if defined(CONFIG_SUPPORT_MASK_LAYER)
	if (decon && decon->current_mask_layer) {
		dsim_wait_for_cmd_done(lcd->dsim);
...
static int s6e8fc1_read_id(struct lcd_info *lcd)
{
...
static int s6e8fc1_read_elvss(struct lcd_info *lcd)
{
	int ret = 0;
	...
	SEQ_ELVSS_SET[LDI_PARA_ELVSS_OTP1 + 1] = buf[LDI_PARA_ELVSS_OTP1];
	SEQ_ELVSS_SET[LDI_PARA_ELVSS_OTP2 + 1] = buf[LDI_PARA_ELVSS_OTP2];
	SEQ_ELVSS_SET[LDI_PARA_ELVSS_OTP3 + 1] = buf[LDI_PARA_ELVSS_OTP3];

	return ret;
}

static int s6e8fc1_read_coordinate(struct lcd_info *lcd)
{
	int ret = 0;
...
static int s6e8fc1_read_chip_id(struct lcd_info *lcd)
{
	int ret = 0;
...
static int s6e8fc1_read_rdnumed(struct lcd_info *lcd)
{
	int ret = 0;
...
static int s6e8fc1_read_esderr(struct lcd_info *lcd)
{
	int ret = 0;
...
static int s6e8fc1_read_rddsdr(struct lcd_info *lcd)
{
	int ret = 0;
...
static int s6e8fc1_read_rddpm(struct lcd_info *lcd)
{
	int ret = 0;
...
static int s6e8fc1_read_rddsm(struct lcd_info *lcd)
{
	int ret = 0;
...
static int s6e8fc1_read_init_info(struct lcd_info *lcd)
{
	...
	s6e8fc1_read_coordinate(lcd);
	s6e8fc1_read_chip_id(lcd);
	s6e8fc1_read_elvss(lcd);
	DSI_WRITE(SEQ_TEST_KEY_OFF_F0, ARRAY_SIZE(SEQ_TEST_KEY_OFF_F0));

	return ret;
}

static int s6e8fc1_exit(struct lcd_info *lcd)
{
	int ret = 0;
...
#if defined(CONFIG_DISPLAY_USE_INFO)
	s6e8fc1_read_rdnumed(lcd);

	DSI_WRITE(SEQ_TEST_KEY_ON_F0, ARRAY_SIZE(SEQ_TEST_KEY_ON_F0));
	s6e8fc1_read_esderr(lcd);
	DSI_WRITE(SEQ_TEST_KEY_OFF_F0, ARRAY_SIZE(SEQ_TEST_KEY_OFF_F0));
#endif
...
static int s6e8fc1_displayon(struct lcd_info *lcd)
{
	int ret = 0;
...
static int s6e8fc1_init(struct lcd_info *lcd)
{
	...
	/* 9.1.3 SEQ_ERR_FG_LINE_COPY_ON_SET */
	DSI_WRITE(SEQ_ERR_FG_LINE_COPY_ON_SET, ARRAY_SIZE(SEQ_ERR_FG_LINE_COPY_ON_SET));
	DSI_WRITE(SEQ_PCD_SET, ARRAY_SIZE(SEQ_PCD_SET));
	DSI_WRITE(SEQ_DISPCTL_SET, ARRAY_SIZE(SEQ_DISPCTL_SET));

	/* ELVSS_DIM_OFF */
	DSI_WRITE(SEQ_ELVSS_DIM_OFF, ARRAY_SIZE(SEQ_ELVSS_DIM_OFF));
...
static int s6e8fc1_register_notifier(struct lcd_info *lcd)
{
...
static int s6e8fc1_probe(struct lcd_info *lcd)
{
	int ret = 0;
	...
	ret = s6e8fc1_read_init_info(lcd);
	if (ret < 0)
...
struct dsim_lcd_driver s6e8fc1_mipi_lcd_driver = {
	.name		= "s6e8fc1",
