...
#ifdef ENABLE_REMOSAIC_CAPTURE
	/*
	 * HACK: Happtn to CSIS_ERR_DMAFIFO_FULL when remosaic capture
	 * so setting to MIF MAX level when 16MB mode change
	 */
	resol = width * height;
	if (device->position == SENSOR_POSITION_FRONT && resol >= SIZE_UHD) {
