...
static void simulate_PINFO(char *arg);
...
static void simulate_PINFO(char *arg)
{
	u64 addr;
	unsigned int val;
	unsigned long *frame_addr = __builtin_frame_address(0);

	pr_crit("%s()\n", __func__);

	addr = virt_to_phys(frame_addr);

	pr_crit("%s: to INFORM8: 0x%x\n", __func__, lower_32_bits(addr));
	exynos_pmu_write(EXYNOS_PMU_INFORM8, lower_32_bits(addr));

	pr_crit("%s: to INFORM9: 0x%x\n", __func__, upper_32_bits(addr));
	exynos_pmu_write(EXYNOS_PMU_INFORM9, upper_32_bits(addr));

	exynos_pmu_read(EXYNOS_PMU_INFORM8, &val);
	pr_crit("%s: INFORM8: 0x%x\n", __func__, val);

	exynos_pmu_read(EXYNOS_PMU_INFORM9, &val);
	pr_crit("%s: INFORM9: 0x%x\n", __func__, val);

	panic("simulate_pinfo");
}
