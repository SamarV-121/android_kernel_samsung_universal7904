...
static void run_hfdnd_read_all(void *device_data)
{
	struct sec_cmd_data *sec = (struct sec_cmd_data *)device_data;
	struct bt532_ts_info *info = container_of(sec, struct bt532_ts_info, sec);
	struct i2c_client *client = info->client;
...
static void run_selfdnd_read(void *device_data)
{
	...
	ret = ts_set_touchmode(DEF_RAW_SELF_SFR_UNIT_DATA_MODE);
	if (ret < 0) {
		ts_set_touchmode(TOUCH_POINT_MODE);
		goto out;
	}
...
static void get_pat_information(void *device_data)
{
	...
	snprintf(cbuff, sizeof(cbuff), "P%02XT%04X", 
				info->cap_info.cal_count, info->cap_info.tune_fix_ver);
...
static void ium_r_write(void *device_data)
{
	...
	set_tsp_nvm_data(info, sec->cmd_param[0], (u8 *)&sec->cmd_param[0], 2);
...
static struct sec_cmd sec_cmds[] = {
	...
	{SEC_CMD("get_dnd_all_data", get_dnd_all_data),},
	...
	{SEC_CMD("run_hfdnd_read_all", run_hfdnd_read_all),},
			 ...
			 struct bt532_ts_platform_data *pdata)
{
	...
	if (ret) {
		input_info(true, dev, "Unable to read pat_function\n");
	...
	if (ret) {
		input_info(true, dev, "Unable to read afe_base\n");
		pdata->afe_base = 0;
	} else
